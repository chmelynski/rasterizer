...
interface Font {
	...
	draw(ctx: Raster, text: string, x: number, y: number, fontSize: number, options: { kerning?: boolean }): void;
	drawPoints(ctx: Raster, text: string, x: number, y: number, fontSize: number, options: { kerning?: boolean }): void;
	drawMetrics(ctx: Raster, text: string, x: number, y: number, fontSize: number, options: { kerning?: boolean }): void;
...
interface Glyph {
	...
	draw(ctx: Raster, x: number, y: number, fontSize: number): void;
	drawPoints(ctx: Raster, x: number, y: number, fontSize: number): void;
	drawMetrics(ctx: Raster, x: number, y: number, fontSize: number): void;
}
interface Path {
...
class Raster {
	...
	lineWidth: number = 1;
	_fillStyle: string = 'rgb(0,0,0)'; // make these objects and parse in the setter?
	_strokeStyle: string = 'rgb(0,0,0)'; // make these objects and parse in the setter?
	lineDashArray: number[] = []; // setLineDash([onPixels, offPixels, onPixels, offPixels, ...])
	lineDashOffset: number = 0;
	lineJoin: string = 'miter'; // miter (default), bevel, round
	lineCap: string = 'butt'; // butt (default), round, square
	miterLimit: number = 10; // is this part of the spec or just a chrome thing?  what is the default miter limit for PDF?
	globalAlpha: number = 1.0; // float in [0,1] - 0 = transparent, 1 = opaque
	globalCompositeOperation: string = 'source-over'; // source-over (default), source-in, source-out, source-atop, destination-over, destination-in, destination-out, destination-atop, lighter, copy, xor (darker was removed from the spec)
	...
	getLineDash(): number[] { return this.lineDashArray; }
	setLineDash(value: number[]): void { this.lineDashArray = value; }
	
	// ----------- fonts
	
	static fontNameToFontObject: Dict<Font> = {}; // must be filled prior to setting ctx.font
	...
	alignText(text: string): { dxPx: number; dyPx: number } {
	...
	// ----------- before this line is the canvas compatibility stuff, after is the raster-specific stuff
	...
	mask: number[] = [ 0x0001, 0x0002, 0x0004, 0x0008, 0x0010, 0x0020, 0x0040, 0x0080, 0x0100, 0x0200, 0x0400, 0x0800, 0x1000, 0x2000, 0x4000, 0x8000 ];
	n: number;
	nn: number;
	//samples: Uint8Array; // width * height * nn, value of {0,1}
	samples: Uint16Array; // width * height, either 9 or 16 packed bits
	...
	closePath(): void {
		this.splines.push({type:SplineType.Line,points:[{x:this.currentPoint.x,y:this.currentPoint.y},{x:this.startPoint.x,y:this.startPoint.y}]});
	...
	lineTo(x: number, y: number): void {
		this.splines.push({type:SplineType.Line,points:[{x:this.currentPoint.x,y:this.currentPoint.y},{x:x,y:y}]});
		this.currentPoint.x = x;
		this.currentPoint.y = y;
	}
	quadraticCurveTo(x1: number, y1: number, x: number, y: number): void {
		this.splines.push({type:SplineType.Quadratic,points:[{x:this.currentPoint.x,y:this.currentPoint.y},{x:x1,y:y1},{x:x,y:y}]});
		this.currentPoint.x = x;
		this.currentPoint.y = y;
	}
	bezierCurveTo(x1: number, y1: number, x2: number, y2: number, x: number, y: number): void {
		this.splines.push({type:SplineType.Cubic,points:[{x:this.currentPoint.x,y:this.currentPoint.y},{x:x1,y:y1},{x:x2,y:y2},{x:x,y:y}]});
		this.currentPoint.x = x;
		this.currentPoint.y = y;
	}
	arcTo(r: number, x1: number, y1: number, x2: number, y2: number): void { }
	fill(): void {
		...
		for (var yPix = bbox.yMin; yPix <= bbox.yMax; yPix++)
		{
			for (var i = 0; i < this.n; i++)
			{
				...
				for (var k = 0; k < xs.length; k += 2)
				{
					...
					if (x1 < 0 || x0 > this.width) { continue; } // discard underflow and overflow
					
					var xPix0 = Math.floor(x0);
					var xPix1 = Math.ceil(x1);
					
					if (xPix0 < 0) { xPix0 = 0; } // left side straddle
					if (xPix1 >= this.width) { xPix1 = this.width - 1; } // right side straddle
	...
	stroke(): void {
		...
		for (var i = 0; i < this.splines.length; i++)
		{
			...
			if (n == 0) { n = 1; } // even tiny line segments get at least one point
			
			if (spline.type == SplineType.Arc)
			{
				for (var k = 0; k <= n; k++)
				{
					var t = k / n;
					...
					if (x < -margin || x > (this.width + margin) || y < -margin || y > (this.height + margin)) { continue; }
				...
				for (var xPix = xFlor; xPix <= xCeil; xPix++)
				{
					...
					for (var i = 0; i < this.n; i++)
					{
						for (var j = 0; j < this.n; j++)
						{
							...
							if ((this.samples[sampleIndex] & this.mask[maskIndex]) > 0) { maskIndex++; continue; }
							...
							if (dd < rr) { this.samples[sampleIndex] |= this.mask[maskIndex]; }
	...
	applyColor(bbox: Bbox, color: Color): void {
		...
		for (var y = bbox.yMin; y <= bbox.yMax; y++)
		{
			...
			for (var x = bbox.xMin; x <= bbox.xMax; x++)
			{
				...
				for (var k = 0; k < this.nn; k++) { sum += (((this.samples[sampleIndex] & this.mask[k]) > 0) ? 1 : 0); }
	...
	rect(left: number, top: number, width: number, height: number): void {
		this.splines.push({type:SplineType.Line,points:[{x:left,y:top},{x:left+width,y:top}]});
		this.splines.push({type:SplineType.Line,points:[{x:left+width,y:top},{x:left+width,y:top+height}]});
		this.splines.push({type:SplineType.Line,points:[{x:left+width,y:top+height},{x:left,y:top+height}]});
		this.splines.push({type:SplineType.Line,points:[{x:left,y:top+height},{x:left,y:top}]});
	}
	arc(cx: number, cy: number, r: number, startAngle: number, endAngle: number, bAntiClockwise: boolean): void {
		...
		if (startAngle >= endAngle) { return; } // startAngle must be less than endAngle
		...
		this.splines.push({type:SplineType.Arc,center:{x:cx,y:cy},radius:r,startAngle:startAngle,endAngle:endAngle});
	}
	...
	setPixel(x: number, y: number, color: Color): void {
		...
		if (0 <= x && x < this.width && 0 <= y && y < this.height)
		{
			...
			//var index = (y * this.width + x) * this.bytesPerPixel;
			//this.pixels[index + 0] = blend.r;
			//this.pixels[index + 1] = blend.g;
			//this.pixels[index + 2] = blend.b;
			//if (this.bytesPerPixel == 4) { this.pixels[index + 3] = 255; }
			////if (this.bytesPerPixel == 4) { this.pixels[index + 3] = color.a; }
		}
	}
	...
	drawImageImpl(image: Bitmap, dx: number, dy: number, dw: number, dh: number, sx: number, sy: number, sw: number, sh: number): void {
...
function SplineCoeffs(spline: Spline, axis: string): number[] {
	
	var coeffs = null;
	
	if (spline.type == SplineType.Line)
	{
		coeffs = LineCoeffs(spline.points[0][axis], spline.points[1][axis]);
	}
	else if (spline.type == SplineType.Quadratic)
	{
		coeffs = QuadCoeffs(spline.points[0][axis], spline.points[1][axis], spline.points[2][axis]);
	}
	else if (spline.type == SplineType.Cubic)
	{
...
function SplineEval(spline: Spline, t: number, coeffs: number[]): number {
...
function LineCoeffs(a: number, b: number): number[] { return [ -a+b, a ]; }
function QuadCoeffs(a: number, b: number, c: number): number[] { return [ a-2*b+c , -2*a+2*b , a ]; }
function BezzCoeffs(a: number, b: number, c: number, d: number): number[] { return [ -a+3*b-3*c+d , 3*a-6*b+3*c , -3*a+3*b , a ]; }
function LineEval(t: number, a: number, b: number): number { return a*t + b; }
function QuadEval(t: number, a: number, b: number, c: number): number { return a*t*t + b*t + c; }
function BezzEval(t: number, a: number, b: number, c: number, d: number): number { return a*t*t*t + b*t*t + c*t + d; }

function SplineIntersections(splines: Spline[], y: number): number[] {
	...
	for (var i = 0; i < splines.length; i++)
	{
		...
		else if (spline.type == SplineType.Arc)
		{
	...
	if (xs.length % 2 == 1)
	{
		...
		//throw new Error('Odd number of intersections at y=' + y);
	}
	
	xs.sort(function(a, b) { if (a < b) { return -1; } else { return 1; } });
...
function ArcHoriIntersections(arc: Spline, y: number, xs: number[]): boolean {
	...
	if (y <= tp || bt <= y) { return false; } // the equal to test will exclude single-point-of-contact at the zenith or nadir
	...
	if (angle1 < 0) { angle1 += Math.PI * 2; } // our angle interval starts at the east compass point and goes counterclockwise
	...
	{
		if (arc.startAngle <= angle1 && angle1 < arc.endAngle) { xs.push(x1); }
		if (arc.startAngle <= angle2 && angle2 < arc.endAngle) { xs.push(x2); }
	}
	else // if it straddles the E compass point, change && to ||
	{
		if (arc.startAngle <= angle1 || angle1 < arc.endAngle) { xs.push(x1); }
		if (arc.startAngle <= angle2 || angle2 < arc.endAngle) { xs.push(x2); }
	}
...
function QuadraticRoots(a: number, b: number, c: number): number[] {
	if (Math.abs(a) < 0.000001) { return LinearRoots(b, c); }
...
function CubicRoots(a: number, b: number, c: number, d: number): number[] {
	...
	if (Math.abs(a) < 0.000001) { return QuadraticRoots(b, c, d); } // since the a coeff is the result of a floating point calc, need some tolerance
...
function LineToSplines(line: Spline, lineWidth: number): Spline[] {
	...
	var splineA = { type: SplineType.Line, points: ap };
	var splineB = { type: SplineType.Line, points: bp };
	var arcA = { type: SplineType.Arc, center: p[0], radius: r, startAngle: a, endAngle: b }; // not sure on the angles here
	var arcB = { type: SplineType.Arc, center: p[1], radius: r, startAngle: b, endAngle: a }; // not sure on the angles here
	
	return [splineA, splineB, arcA, arcB];
}
function StrokeMesh(lines: Spline[]): void {
...
const enum SplineType { Line, Quadratic, Cubic, Arc }
interface Point {
	x: number;
	y: number;
}
...
function BoundingBox(raster: Raster, splines: Spline[]): Bbox {
	...
	for (var i = 0; i < splines.length; i++)
	{
		...
		if (spline.type == SplineType.Arc)
		{
			...
			if (a < b)
			{
				if (a < N && N < b) { yMin = spline.center.y - spline.radius; }
				if (a < E && E < b) { xMax = spline.center.x + spline.radius; }
				if (a < S && S < b) { yMax = spline.center.y + spline.radius; }
				if (a < W && W < b) { xMin = spline.center.x - spline.radius; }
			}
			else // straddles the E pole
			{
				if (a < N || N < b) { yMin = spline.center.y - spline.radius; }
				if (a < E || E < b) { xMax = spline.center.x + spline.radius; }
				if (a < S || S < b) { yMax = spline.center.y + spline.radius; }
...
function DebugPixels(ctx: Raster): string {
	
	function PadLeft(k, n) {
		var str = k.toString();
		var padding = '';
		for (var i = 0; i < n - str.length; i++) { padding += '0'; }
		return padding + str;
	}
...
function DebugSamples(ctx: Raster): string {
...
function DebugSplines(splines: Spline[]): void {
	...
	for (var i = 0; i < splines.length; i++)
	{
		...
		if (i % 10 == 0) { tr = document.createElement('tr'); table.appendChild(tr); }
